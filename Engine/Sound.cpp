...
#define CUSTOM_SOUND_API_EXCEPTION( hr,note ) SoundSystem::APIException( hr,_CRT_WIDE(__FILE__),__LINE__,note )
#define CUSTOM_SOUND_FILE_EXCEPTION( filename,note ) SoundSystem::FileException( _CRT_WIDE(__FILE__),__LINE__,note,filename )
...
void SoundSystem::SetMasterVolume(float vol)
{
	...
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Setting master volume");
	}
}
	...
	while (true)
	{
		...
		else
		{
			...
			case LoadType::Folder:
				...
				break;
			case LoadType::Local:
				throw CUSTOM_SOUND_API_EXCEPTION(
	...
	format(std::make_unique<WAVEFORMATEX>())
{
	...
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Creating factory for XAudio2 object");
	}
	...
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Creating XAudio2 object");
	}

	// initialize the XAudio2 component object
	if (FAILED(hr = pEngine->Initialize(0, XAUDIO2_DEFAULT_PROCESSOR)))
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Initializing XAudio2 object");
	}

	// create the mastering voice
	if (FAILED(hr = pEngine->CreateMasteringVoice(&pMaster)))
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Creating mastering voice");
	}
	...
	xaBuffer(std::make_unique<XAUDIO2_BUFFER>())
{
	...
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Creating source voice for channel");
	}
}
...
void SoundSystem::Channel::PlaySoundBuffer(Sound& s, float freqMod, float vol)
{
	...
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Starting playback - submitting source buffer");
	}
	if (FAILED(hr = pSource->SetFrequencyRatio(freqMod)))
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Starting playback - setting frequency");
	}
	if (FAILED(hr = pSource->SetVolume(vol)))
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Starting playback - setting volume");
	}
	if (FAILED(hr = pSource->Start()))
	{
		throw CUSTOM_SOUND_API_EXCEPTION(hr, L"Starting playback - starting");
	}
}
	...
	float loopStartSeconds, float loopEndSeconds)
{
	...
	{
		{
...

			{
				...
				{
					throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"Bad fourcc code");
				}
			}
			...
			if (fileSize <= 44u)
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"file too small");
			}
		...
		{
			throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"format not WAVE");
		}
		...
		if (!bFilledFormat)
		{
			throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"fmt chunk not found");
		}
		...
		// compare format with sound system format
		{
			...
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"bad wave format (nChannels)");
			}
			else if (format.wBitsPerSample != sysFormat.wBitsPerSample)
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"bad wave format (wBitsPerSample)");
			}
			else if (format.nSamplesPerSec != sysFormat.nSamplesPerSec)
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"bad wave format (nSamplesPerSec)");
			}
			else if (format.wFormatTag != sysFormat.wFormatTag)
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"bad wave format (wFormatTag)");
			}
			else if (format.nBlockAlign != sysFormat.nBlockAlign)
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"bad wave format (nBlockAlign)");
			}
			else if (format.nAvgBytesPerSec != sysFormat.nAvgBytesPerSec)
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"bad wave format (nAvgBytesPerSec)");
			}
		}
		...
		if (!bFilledData)
		{
			throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"data chunk not found");
		}
		...
		case LoopType::AutoEmbeddedCuePoints:
		{
			...
			if (!bFilledCue)
			{
				throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, L"loop cue chunk not found");
			}
		}
		break;
	...
	catch (const std::exception& e)
	{
		...
		throw CUSTOM_SOUND_FILE_EXCEPTION(fileName, std::wstring(what.begin(), what.end()));
	}
}
